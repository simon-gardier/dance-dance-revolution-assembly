...
update_music:
    ...
    goto    end_update_music_no_note   ; then goto next iteration

    movf    music_note_index, 0             ; else get the index of the note to play
    sublw   TETRIS_SONG_LENGTH                    ; (we do TETRIS_SONG_LENGTH - music_note_index, because music_note_index starts at TETRIS_SONG_LENGTH)
    ...
    movlw   TETRIS_SONG_LENGTH                    ; else reset music_note_index to TETRIS_SONG_LENGTH
...
reset_music_note_iteration_counter:
    ...
    goto    end_update_music_no_note   ; then return
    ...
    return
end_update_music_no_note:
    retlw   4

; ------------------------------------------------------------------------------
; Local Look up table tetris_music_notes() storing the indexes of the notes producing the music "Korobeniki" (i.e. Tetris theme)
...
; ------------------------------------------------------------------------------
; Local Look up tables for the notes
...
; ------------------------------------------------------------------------------
; Local Lookup table note_data_index_to_arrow_column_index()
